...
class StableSelector:
    ...
    def draw_body_geometry(self, rr, joints, thickness=0.03, shrink=0.5):
        ...
        for i, c in enumerate(cuboids):
            quat = Rotation.from_matrix(c["rotation"]).as_quat()
            rr.log(
                f"body_cuboid/{i}",
                rr.Boxes3D(
                    centers=[c["center"]],
...
class StableSelectorV2:
    ...
    #     source_data = {}
    #     for sid in sources:
    #         subgroups = list(f[f"poses_raw/{tid}/{sid}"].keys())
    #         if not subgroups:
    #             continue
    #         sg = subgroups[0]  # usually '0'
    #         base = f[f"poses_raw/{tid}/{sid}/{sg}"]
    #         source_data[sid] = {
    ...
    #     # Merge all available timestamps
    ...
    #             if abs(ts_array[idx] - t) > 0.05:  # skip if too far (50 ms)
    #                 continue
    ...
    #         rr.log(f"stable_selector/frame_{int(t*100):06d}/chosen_sensor", rr.TextLog(f"Sensor {best_sid}"))
    #         rr.log(f"stable_selector/frame_{int(t*100):06d}/costs", rr.Scalars({k: v for k, v in frame_costs.items()}))
